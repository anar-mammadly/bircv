...
export default function TemplatePreview({ template, lang, font, width }: {
  template: TemplateId; lang: 'az' | 'en'; font?: CVFontId; width?: number;
}) {
  ...
  return (
    <div ref={ref} className="relative w-full overflow-hidden bg-white" style={{ aspectRatio: '210 / 297' }}>
      {visible && w > 0 ? (
        <CVDocument data={lang === 'az' ? DEMO_CV_AZ : DEMO_CV_EN} template={template} lang={lang} font={font} fixedWidth={w} maxPages={1} showPageNumbers={false} flat />
      ) : (
